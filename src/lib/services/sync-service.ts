...
class SyncService {
  ...
  private isPermanentOrderSyncError(error: unknown): boolean {
    const message = String(error ?? '').trim().toLowerCase();
    if (!message) return false;

    return [
      'credit limit exceeded',
      'customer account is not enabled',
      'on account sales require',
      'laybuy sales require',
      'laybuy deposit cannot exceed',
      'available for',
      'after existing laybuy reservations',
    ].some((signature) => message.includes(signature));
  }

  private async markOrderSyncRejected(id: string, error: unknown): Promise<void> {
    const message = String(error ?? 'Order was rejected by the server.');
    ...
    console.warn(`[Sync] Marked order ${id} as rejected: ${message}`);
  }
  ...
  private normalizeCustomerRecord(record: any, fallbackBranchId = ''): any | null {
    ...
    const currentBalance = this.toNumber(converted.currentBalance ?? record?.current_balance, 0);
    const creditLimit = this.toNumber(converted.creditLimit ?? record?.credit_limit, 0);
    ...
    return {
      ...converted,
      id,
      businessId: businessId || converted.businessId,
      branchId,
      name: String(converted.name ?? record?.name ?? '').trim() || 'Unnamed Customer',
      email: String(converted.email ?? record?.email ?? '').trim(),
      phone: String(converted.phone ?? record?.phone ?? '').trim(),
      address: String(converted.address ?? record?.address ?? '').trim(),
      notes: String(converted.notes ?? record?.notes ?? '').trim(),
      isActive: this.toBoolean(converted.isActive ?? record?.is_active, true),
      accountEnabled: this.toBoolean(converted.accountEnabled ?? record?.account_enabled, true),
      creditLimit,
      currentBalance,
      availableCredit: availableCreditRaw === null || availableCreditRaw === undefined
        ? null
        : this.toNumber(availableCreditRaw, 0),
      hasCreditLimit: this.toBoolean(converted.hasCreditLimit ?? record?.has_credit_limit, creditLimit > 0),
      customerTin,
  ...
  private async pushChanges(
    ...
    try {
      ...
      let orderChanges = changes.filter(c => c.entity_type === 'Order');
      ...
      const pushCustomerChanges = async (): Promise<boolean> => {
        ...
        try {
          ...
          if (acknowledged.length > 0) {
            console.log(`[Sync] ${acknowledged.length} customer changes acknowledged`);
            for (const ack of acknowledged) {
              await this.applyCustomerSyncAck(ack);
            }
          }

          const errors = Array.isArray(result.results?.errors) ? result.results.errors : [];
          if (errors.length > 0) {
            console.error(`[Sync] ${errors.length} customer sync errors:`, errors);
            return false;
          }

          if (acknowledged.length < customerChanges.length) {
            console.warn('[Sync] Customer sync acknowledged fewer changes than expected.');
            return false;
          }

          return true;
        } catch (error) {
          console.error('[Sync] Customer push failed:', error);
          return false;
        }
      };
      ...
      // Push customers before orders so offline loyalty/account sales can reference server customer IDs.
      const customerPushSucceeded = await pushCustomerChanges();
      if (!customerPushSucceeded) {
        console.warn('[Sync] Customer sync incomplete; skipping order push for this cycle.');
        return;
      }
      if (customerChanges.length > 0 && orderChanges.length > 0) {
        const refreshedChanges = await this.collectLocalChanges(branchId);
        orderChanges = refreshedChanges.filter(c => c.entity_type === 'Order');
      }
        ...
        try {
          ...
          if (result.results?.errors && result.results.errors.length > 0) {
            ...
            for (const syncError of result.results.errors) {
              if (syncError?.id && this.isPermanentOrderSyncError(syncError.error)) {
                await this.markOrderSyncRejected(syncError.id, syncError.error);
              }
            }
          }
          ...
          const retryableErrors = errors.filter(
            (syncError: any) => !this.isPermanentOrderSyncError(syncError?.error)
          );
          const retryableOrderChanges = orderChanges.filter((change) => (
            !errors.some((syncError: any) => (
              String(syncError?.id) === String(change.id) &&
              this.isPermanentOrderSyncError(syncError?.error)
            ))
          ));

          if (retryableErrors.length > 0 || acknowledged.length < retryableOrderChanges.length) {
      ...
      for (const customer of customers) {
        if (customer._dirty) {
          changes.push({
            ...
            op: customer._operation || 'update',
            data: this.sanitizeForSync(customer),
            timestamp: new Date().toISOString()
          });
        }
      }

      console.log(`[Sync] Collected ${customers.filter(c => c._dirty).length} dirty customers`);
  ...
  private async applyCustomerSyncAck(ack: any): Promise<void> {
    const localId = String(ack?.id ?? '').trim();
    const serverId = String(ack?.server_id ?? ack?.serverId ?? '').trim();
    const fallbackId = localId || serverId;
    ...
    try {
      const localCustomer = localId ? await db.customers.get(localId) : null;
      ...
      if (localCustomer._operation === 'delete') {
        await db.customers.delete(localId);
        if (serverId && serverId !== localId) {
          await db.customers.delete(serverId);
        }
        console.log(`[Sync] Removed deleted customer ${localId} after backend acknowledgement`);
        return;
      }

      const normalizedAck = this.normalizeCustomerRecord(
        ack,
        String(localCustomer.branchId || '')
      );
      const resolvedServerId = String(serverId || normalizedAck?.id || localId);

      const mergedCustomer = {
        ...localCustomer,
        ...(normalizedAck || {}),
        id: resolvedServerId,
        branchId: normalizedAck?.branchId || localCustomer.branchId,
        _dirty: false,
        _operation: undefined,
        _synced_at: nowIso,
      };

      await db.customers.put(mergedCustomer);
      if (localId && resolvedServerId !== localId) {
        await db.customers.delete(localId);

        const ordersWithLocalCustomerId = (await db.orders.toArray()).filter((order) => {
          const orderCustomerId = String(order.customerId ?? '').trim();
          const orderCustomerSnakeId = String(order.customer_id ?? '').trim();
          return orderCustomerId === localId || orderCustomerSnakeId === localId;
        });

        for (const order of ordersWithLocalCustomerId) {
          await db.orders.update(order.id, {
            customerId: resolvedServerId,
            customer_id: resolvedServerId,
          });
        }
      }

      console.log(`[Sync] Marked customer ${resolvedServerId} as synced`);
    } catch (error) {
      console.error(`[Sync] Failed to apply customer acknowledgement for ${fallbackId}:`, error);
      await this.markChangeAsSynced(fallbackId);
    }
  }
  ...
  private async markChangeAsSynced(id: string): Promise<void> {
    ...
    try {
      ...
      if (customer) {
        if (customer._operation === 'delete') {
          await db.customers.delete(id);
          console.log(`[Sync] Removed deleted customer ${id}`);
        } else {
          await db.customers.update(id, { _dirty: false, _operation: undefined });
          console.log(`[Sync] Marked customer ${id} as synced`);
        }
        return;
      }
    } catch (error) {
  ...
  private async applyServerChanges(changes: any, branchId?: string): Promise<void> {
    ...
    try {
      ...
      // Apply customers and account balances
      if (changes.customers && Array.isArray(changes.customers) && changes.customers.length > 0) {
        const fallbackBranchId = branchId ? String(branchId).trim() : '';

        for (const customer of changes.customers) {
          try {
            const normalizedCustomer = this.normalizeCustomerRecord(customer, fallbackBranchId);
            if (!normalizedCustomer) {
              console.warn('[Sync] Skipping customer without id:', customer);
              continue;
            }

            const existingCustomer = await db.customers.get(normalizedCustomer.id);
            if (existingCustomer?._dirty) {
              console.log(`[Sync] Skipping server overwrite for dirty customer ${normalizedCustomer.id}`);
              continue;
            }
            ...
            await db.customers.put({
              ...(existingCustomer || {}),
              ...normalizedCustomer,
              branchId: normalizedCustomer.branchId || existingCustomer?.branchId || fallbackBranchId,
              _dirty: false,
              _operation: undefined,
              _synced_at: new Date().toISOString()
            });
          } catch (error) {
            console.error(`[Sync] Error applying customer ${customer.id}:`, error);
          }
        }

        console.log(`[Sync] Applied ${changes.customers.length} customer changes`);
      }
