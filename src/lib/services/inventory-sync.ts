...
function normalizeInventoryProduct(
  ...
  const reservedStockUnits = toOptionalNumber(
    converted.reservedStockUnits ??
    backendProduct?.reserved_stock_units ??
    existingProduct?.reservedStockUnits
  ) ?? 0;
  const availableStockUnits = toOptionalNumber(
    converted.availableStockUnits ??
    backendProduct?.available_stock_units ??
    existingProduct?.availableStockUnits
  ) ?? Math.max(0, stockUnits - reservedStockUnits);
  ...
  const portionsPerUnit = isSoldInPortions
    ...
    : undefined;
  const portionPrice = isSoldInPortions
    ? toOptionalNumber(
        converted.portionPrice ??
        backendProduct?.portion_price ??
        existingProduct?.portionPrice
      )
    : undefined;
  ...
  return {
    ...
    category: String(converted.category ?? existingProduct?.category ?? '').trim(),
    itemType,
    stockUnits,
    reservedStockUnits,
