...
import { db, type Order, type Refund, type OrderItem, type Session } from '@/lib/db';
...
const getOrderCollectionAmounts = (order: Order) => {
    const grossValue = toFiniteNumber(
        order.total ?? (order as any).grossAmount ?? (order as any).gross_amount,
        0
    );
    const paymentMethod = toTrimmedString(order.paymentMethod).toLowerCase();
    ...
    if (paymentMethod === 'laybuy') {
        const deposit = Math.max(
            ...
            Math.min(
                grossValue,
                toFiniteNumber(
                    (order as any).laybuyDeposit ??
                    (order as any).laybuy_deposit ??
                    (order as any).depositAmount ??
                    (order as any).deposit_amount,
                    0
                )
            )
        );
...
export default function ReportsPage() {
    ...
    const sessionsInRange = useLiveQuery(async () => {
        if (!activeBranchId) return undefined;
        const sessions = await db.sessions
            ...
            .and((session: Session) => {
                ...
                return startedAt >= Date.parse(fromDate) && startedAt <= Date.parse(toDate);
            })
            .toArray();
        return sessions;
    }, [activeBranchId, fromDate, toDate]);
    ...
    const reportSummary = useMemo(() => {
        return calculateZReportSummary((activeOrders || []) as any);
    }, [activeOrders]);

    const paymentBreakdown = reportSummary.paymentBreakdown;

    const sessionCollectionBreakdown = useMemo(() => {
        if (!sessionsInRange || sessionsInRange.length === 0) return null;
    ...
    const displayedPaymentBreakdown = sessionCollectionBreakdown || paymentBreakdown;
    ...
    const kpiMetrics = [
        { title: 'Sales Value', value: formatCurrency(data.totalRevenue), icon: DollarSign },
        ...
        { title: 'Transactions', value: data.totalTransactions, icon: ShoppingCart },
        { title: 'Avg Sale', value: formatCurrency(data.averageOrderValueWithTax), icon: Landmark },
    ];
      ...
      <Tabs defaultValue="sales" className="w-full">
        ...
        <TabsContent value="sales">
            <div className="grid gap-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Collections & Amounts Due</CardTitle>
                        <CardDescription>Money received versus invoice and laybuy balances for the selected period.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                <div className="rounded-lg border bg-muted/30 p-4">
                                    ...
                                    <div className="mt-1 text-lg font-bold text-green-700">{formatCurrency(displayedPaymentBreakdown.totalCollected)}</div>
                                </div>
                                <div className="rounded-lg border bg-muted/30 p-4">
                                    <div className="text-xs font-medium text-muted-foreground">Still Due</div>
                                    <div className="mt-1 text-lg font-bold text-amber-700">{formatCurrency(displayedPaymentBreakdown.totalDue)}</div>
                                </div>
                                {collectionRows.length === 0 ? (
                                    <div className="rounded-lg border bg-muted/30 p-4 sm:col-span-2">
                                        <div className="text-sm text-muted-foreground">No collections or balances for this period.</div>
                                    </div>
                                ) : collectionRows.map((row) => (
                                    <div key={row.label} className="rounded-lg border bg-muted/30 p-4">
                                        <div className="text-xs font-medium text-muted-foreground">{row.label}</div>
                                        <div className="mt-1 text-lg font-semibold">{formatCurrency(row.value)}</div>
                                    </div>
                                ))}
                            </div>
                        )}
